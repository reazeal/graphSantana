...
class GraphKecepatan_model extends MY_Model
{
    public $table = 'graph2';
	...
	public function getdataKecepatan(){
        $data = array();
		$this->db->select("
        nopol, sum(jarak) as jarak
        ");
		 $this->db->group_by('nopol');
        $query = $this->db->get('jarak_kendaraan');
        ...
        if ($totaly2 > 0) {
            foreach ($query->result() as $atributy) {
                ...
                $data[] = array(
                    (string) 'name' => (string) $atributy->nopol,
					(string) 'drilldown' => (string) $atributy->nopol,
                    (string) 'y' => (double) $atributy->jarak
                );
            }
	...
	public function getdataKecepatanx(){
        $data = array();
		$this->db->select("
        nopol, sum(jarak) as jarak,  waktu
        ");
		 $this->db->group_by('date_format(waktu,"%y-%m-%d"), nopol');
        $query = $this->db->get('jarak_kendaraan');
	...
	public function getdataDrilldownkecepatan($nopol){
        $data = array();
        $this->db->select("
        `nopol`,
        GROUP_CONCAT(CONCAT_WS(';',`waktu`,`jarak`)) AS dt
        ");
    ...
    public function getdataxkecepatanx(){
        $data = array();
        $this->db->select("
        nopol, sum(jarak) as jarak
        ");
        ...
        if ($totaly2 > 0) {
            foreach ($query->result() as $atributy) {
                ...
                $data[] = array(
					 (string) 'id' => (string) $atributy->nopol,
                    (string) 'name' => (string) $atributy->nopol,
                    (string) 'data' => $this->getdataDrilldownkecepatan($atributy->nopol)
                );
            }
