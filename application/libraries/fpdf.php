...
class FPDF {
	...
	function Footer() {
		...
		//$this->Cell(0,38,'* Catatan :* BKP =Bobot Kering Panen,* BKS =Bobot Kering Simpan,* BKG =Bobot Kering Giling, *Per Tgl Panen = Perkiraan Tanggal Panen',0,0,'L');
		//$this->Cell(0,38,'           ',0,0,'L');
		//Page number
		//$this->Cell(0, 38, 'Halaman ke : ' . $this->PageNo(), 0, 0, 'R');
	}
	...
	function RowBorder($data)
	{
		...
		{
			$w=$this->widths[$i];
			$border=isset($this->borders[$i]) ? $this->borders[$i] : '0';
			$a=isset($this->aligns[$i]) ? $this->aligns[$i] : 'L';
			$b=isset($this->colors[$i]) ? $this->colors[$i] : '0';
			//Save the current position
			$x=$this->GetX();
			$y=$this->GetY();
			//Draw the border
			...
			//Print the text
			if(isset($this->colors1[$i])){
			$col1=isset($this->colors1[$i][0]) ? $this->colors1[$i][0] : 255;
			$col2=isset($this->colors1[$i][1]) ? $this->colors1[$i][1] : 255;
			$col3=isset($this->colors1[$i][2]) ? $this->colors1[$i][2] : 255;
			}else {
			$col1=255;
			$col2=255;
			$col3=255;
			...
			$this->MultiCell($w,3,$data[$i],$border,$a,$b);
			//Put the position to the right of the cell
			$this->SetXY($x+$w,$y);
		}
		//Go to the next line
	...
	function RowImage($data,$pos_image,$dir)
	{
		//Calculate the height of the row
		$nb=0;
		for($i=0;$i<count($data);$i++)
		...
		for($i=0;$i<count($data);$i++)
		{
			...
			if($i==($pos_image-1)&&trim($data[$i])){
				$this->Image('./'.$dir.trim($data[$i]),$x,$y,40);
			}
		...
		for ($i = 0; $i < count($data); $i++) {
			...
			$this->MultiCell($w, 0.5, $data[$i], 'LRTB', $a, true);
			//Put the position to the right of the cell
			$this->SetXY($x + $w, $y);
		}
		//Go to the next line
